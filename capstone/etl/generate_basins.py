...
def generate_us_basins(census, eia_cnty, basins_output_dir):
    ...
    basins_geo = pd.merge(left=census, right=eia_cnty, on='geoid', how='inner')

    basins_gdf_list = []

    for region in basins_geo['region'].unique():
        print(f"    {region.lower()}")

        region_gdf = basins_geo[basins_geo['region'] == region]
